...
class StartStopStep(StepBase, BaseColletingStep):
    temperatureSensor = StepProperty.Sensor("Temperature sensor", description="Датчик температуры в царге")
    initialCollecting = Property.Number("Start sampling rate, ml / h", configurable=True, default_value=1000)
    deltaTemp = Property.Number("Allowed temperature entry, °C", configurable=True, default_value=0.3)
    decrement = Property.Number("Decrease in selection when air in temperature,%", configurable=True, default_value=10)
    ...
    def finish(self):
        ...
        self.notify("", "election completion temperature", type="success", timeout=2000)
